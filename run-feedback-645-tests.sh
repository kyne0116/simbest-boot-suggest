...
echo "已更新测试配置文件: src/test/resources/testng-feedback-645.xml"
...
# 设置自定义报告文件名
REPORT_FILE="test-output/feedback-645-report.html"

# 运行测试并指定输出报告文件
java -cp target/test-classes:target/classes:$(mvn dependency:build-classpath -Dmdep.outputFile=/dev/stdout -q) org.testng.TestNG -d test-output/feedback-645 src/test/resources/testng-feedback-645.xml
